  ...
  projectileAttack() {
    ...
    if (this.chargingTimer < chargingLimit) {
      ...
      if (this.projectileTimer < projLimit) {
        ...
        let proj = new SamuraiProjectile(this.game, projX, this.y - this.game.camera.y + 72, 16 * PARAMS.SCALE, this.box.height / 2, this.dir, PROJECTILE_VELOCITY, PROJECTILE_DAMAGE);
      ...
      if (that.box.collide(proj.hitbox)) {
        if (!(proj instanceof SamuraiProjectile)) {
          ...
          if (Math.random() * 1 < that.game.hero.critChance) {
            ...
            that.game.addEntity(new Score(that.game, that.x - that.game.camera.x + that.offset, that.y - that.game.camera.y + 50, critDamage, true));
            that.currentHealth -= proj.damage * critMultiplier;
          } else {
            that.game.addEntity(new Score(that.game, that.x - that.game.camera.x + that.offset, that.y - that.game.camera.y + 50, proj.damage, false));
